...
/** 給非開發者看的權限說明（spec §6.2 逐字要求的內容）：不用「OAuth」「scope」這類術語，
 *  直接講「為什麼」——要幫你 fork 一份、把改動寫進「你自己的」那份、再送出 PR，本站不會
 *  也沒有能力動你其他的 repo。這段文字固定顯示在登入鈕旁（不是藏在展開/tooltip 裡的次要
 *  資訊）：信任門檻要在玩家按下去之前就講清楚，不是按下去、看到 GitHub 授權頁才補救。 */
const PERMISSION_NOTE =
  '按下「用 GitHub 登入」後，GitHub 會問你要不要讓這個網站存取你的公開儲存庫。' +
  '這是因為送出 PR 得先幫你 fork 一份骰子樹的資料，把你的改動寫進「你自己的」那份 fork 裡，' +
  '再拿它去開 PR——本站不會也沒有能力動你其他的 repo。';
