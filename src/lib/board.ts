...
/**
 * 排序方向，站上格子座標：0 上、1 右、2 下、3 左。單盤時四向對稱，方向由使用者照畫面指定。
 * ⚠️ 合作模式下不對稱：客戶端只有**一個**方向會跨到另一盤（對同一欄的整欄 3 格施加），對到畫面上
 * 就是「箭頭指著對方那一盤」的那一個——上面那盤是 2（↓）、下面那盤是 0（↑），兩盤相反。
 * 跨盤那一段與列無關（整欄都打），方向由呼叫端傳給 boardBuffs()，見 board-buffs.ts。
 */
