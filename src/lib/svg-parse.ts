...
/**
 * 讀屬性值。**一律走 `getAttributeNode` 而不是 `getAttribute`**：實測發現 linkedom 的
 * `getAttribute` 對具名實體（`&amp;`／`&lt;`）不解碼，Chromium 會解碼，兩邊讀出來不一樣
 * （根因在 linkedom 的 `Element.getAttribute` 對非 `ignoreCase` 元素——也就是這裡的 SVG／
 * XML 文件——把底層已經正確解碼過的值又重新套一次跳脫，等於多繞一圈繞回原樣）；
 * `getAttributeNode(...).value` 讀的是解碼後、沒被那層多餘跳脫動過的內部值，兩邊一致。
 * 數值實體（`&#10;`）兩邊本來就都正確解碼，但沒有理由為此分兩種讀法讓下一個人猜該用哪個。
 */
...
function shapeOf(g: Element): Shape {
  ...
  if (poly) {
    const pts = attr(poly, 'points').trim().split(/\s+/).length;
...
export function parseTreeWith(
  ...
  const meta: RawMeta = {
    svgVersion: attr(svg, 'data-version'),
    gameBundle: bundle,
    updated: attr(svg, 'data-updated'),
  ...
  const nodes: RawNode[] = [...doc.querySelectorAll('g.node')].map(g => {
    ...
    const [x, y] = parseTranslate(attr(g, 'transform'));
    ...
    const imageEl = g.querySelector('image');
    const href = imageEl ? attr(imageEl, 'href') : '';
  ...
  const edges: RawEdge[] = [...doc.querySelectorAll('path.edge')].map(p => {
    const dAttr = attr(p, 'd');
    if (!attr(p, 'marker-end')) throw new Error(`邊缺少 marker-end，無法判定方向：d="${dAttr}"`);
