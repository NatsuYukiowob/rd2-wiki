...
```bash
npm run validate    # 資料驗證（規則 0–10、13–18，CI 守門員）
...
- 效能預算硬斷言：`tree.json` gzip ≤ 20KB（**目前 19.0KB，只剩 1.0KB**）、sprite ≤ 400KB（目前 130KB）。
...
- **`data/maxlevel-official.json`＝官方資料表標註的 44 條滿級數值**（`Lv.50：216%`），
  鍵是 `data-game-id`。**它不進 tree.json、站台一個字都不顯示**——站台的滿級值是
  ...
  關鍵字，規則 1–16 全部放行。⚠️ **鍵一定要用 `data-game-id` 不能用節點 id 或名稱**：
