...
- **對照鍵**：節點 id ＝ `DiceTreeNodeTable.Id`（`1501`／`1601` 就是上游自己的 id）；成本 ＝
  ...
  **SVG 座標 ＝ `(1000 + 0.2·x, 850 − 0.2·y)`**（`Position` 欄）。2026-09-06 依 Yuki 的實機截圖把火系
  ...
  243 顆裡只剩 `1407` 與 `1601` 不吻合：`1407` 是刻意的版面調整，`1601` 比表格再往下 30（表格把符文
  放在骰子正下方 50 處，本站的骰子標籤就在那裡）。2026-09-18 依 1.1.2 表格搬了鐵甲系 `2201`／`2301`／`2401`
...
- **齒輪二階骰子（`2503`，1.1.2）**：照太陽骰子的模式落地——座標照表格 (830,970)（騰位置的 6 顆符文
  一起搬了，見上面座標那條），符文 `2603` 齒輪二階強化照表格放在 (870,930)（**沒有**比照 `1601` 往下挪：
  它在骰子的右上斜角，不壓骰子標籤）。解鎖另要 `2203` 練到 Lv.50（`prereq-ranks.json`）；登場條件
...
在骰子樹上逐顆解鎖、調等級，即時算出這套規劃要花多少核心、金幣與超越核心（資源上限的輸入框是
`sim-limit-core`／`-gold` ＋ 每種超越核心一格 `sim-limit-<kind>`（由 `sim.astro` 依 `MYTHIC_CORES` 產生），
`exceedsLimit()` 的 `limits` 是 `SimLimits`（`core`／`gold`／`mythic[kind]`，都是 `number | null`），超越核心走同一條
