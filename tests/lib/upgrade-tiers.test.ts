...
describe('upgradeExtraCost', () => {
  ...
  it('special 表帶 solar 時照樣逐級累加', () => {
    const solarRows: LevelCost[] = [
      { level: 2, gold: 100000, core: 0, mythic: { solar: 200 } },
      { level: 3, gold: 150000, core: 0, mythic: { solar: 300 } },
      // 缺席的 solar 當 0——兩份費用表 JSON 的既有列都沒有這個欄位
      { level: 4, gold: 200000, core: 0 },
    ];
    expect(upgradeExtraCost(solarRows, 2)).toEqual({ core: 0, gold: 100000, mythic: { solar: 200 } });
    expect(upgradeExtraCost(solarRows, 3)).toEqual({ core: 0, gold: 250000, mythic: { solar: 500 } });
    expect(upgradeExtraCost(solarRows, 4)).toEqual({ core: 0, gold: 450000, mythic: { solar: 500 } });
  });
