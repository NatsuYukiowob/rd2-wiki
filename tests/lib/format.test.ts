...
describe('format', () => {
  ...
  it('有太陽核心時接在最後，沒有時整段不出現', () => {
    expect(formatCost({ core: 0, gold: 100000, mythic: { solar: 2000 } })).toBe('金幣 100,000 ＋ 太陽核心 2,000');
    expect(formatCost({ core: 12, gold: 50000, mythic: { solar: 100 } })).toBe('核心 12 ＋ 金幣 50,000 ＋ 太陽核心 100');
    expect(formatCost({ core: 0, gold: 0, mythic: { solar: 2000 } })).toBe('太陽核心 2,000');
    expect(formatCost({ core: 26, gold: 12000 })).toBe('核心 26 ＋ 金幣 12,000');
  });
  it('滿級換算', () => {
