...
describe('renderDetail', () => {
  ...
  // 1601 太陽強化曾被誤記成 50 級（實機是 20 級，見客戶端 RuneTable.MaxRank），那時用通用表算
  // 會印出「核心 99 ＋ 金幣 465,700」——一個看起來很專業、跟真實費用差兩個數量級的數字，而
  // 畫面上沒有任何地方說話。這條同時釘住等級上限：maxLevel 一被改回 50 這行的「20 級」就會紅。
  // 算式：special["1601"] 的 Lv.2–20 追加 金幣 10,450,000 ／太陽核心 20,900
  //       ＋ 解鎖那一筆 金幣 50,000 ／太陽核心 100 ＝ 金幣 10,500,000 ／太陽核心 21,000。
  it('1601 太陽強化的練滿累計走 special 表，不是通用符文表', () => {
    const { host } = renderNode('1601');
    const upgrade = host.querySelector('.upgrade')?.textContent ?? '';
    expect(upgrade).toContain('練滿 20 級累計 金幣 10,500,000 ＋ 太陽核心 21,000');
