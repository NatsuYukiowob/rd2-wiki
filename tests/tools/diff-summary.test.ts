...
// 超越核心（2026-09-18 泛化）：base 分支建出來的 tree.json 可能還是 1.1.0 的 `{core, gold, solar}`，
// head 是 `{core, gold, mythic}`——兩種形狀要讀成同一件事，否則改版那個 PR 的摘要會印出「太陽核心 0 → 2,100」。
describe('buildDiffSummary 的超越核心', () => {
  const legacy = (c: Record<string, unknown>) => tree([n({ id: '1001' })], [], c as { core: number; gold: number });

  it('base 是舊形狀 solar、head 是 mythic 時，照樣讀成同一個數字', () => {
    const out = buildDiffSummary(legacy({ core: 1, gold: 2, solar: 2100 }), legacy({ core: 1, gold: 2, mythic: { solar: 2100 } }));
    expect(out).toContain('太陽核心 2,100 → 2,100');
  });

  it('兩邊都沒有超越核心時一個字都不印', () => {
    expect(buildDiffSummary(legacy({ core: 1, gold: 2 }), legacy({ core: 1, gold: 2 }))).not.toContain('核心 0 →');
  });

  // mythic 的鍵來自送 PR 的人：沒登記的鍵一個字都不能進留言。
  it('沒登記的超越核心鍵不會出現在留言裡', () => {
    const out = buildDiffSummary(legacy({ core: 1, gold: 2 }), legacy({ core: 1, gold: 2, mythic: { '<img src=x onerror=alert(1)>': 5 } }));
