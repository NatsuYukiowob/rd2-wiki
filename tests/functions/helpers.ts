...
// 共用同一份路由邏輯——兩邊對「GitHub／raw.githubusercontent.com 回什麼」的假設不會因為
// 各自維護一份而漂移。原本只有 gh.test.ts 一個消費者時這份定義就地寫在那支測試檔裡
// （只有一個消費者時抽共用檔是過早抽象）；Task 20 的 submit.test.ts 成為第二個消費者後才抽出來。
...
export function fakeGitHub(overrides: Record<string, () => Response> = {}) {
  ...
  const f = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    // submit.ts 讀上游 data/keywords.json 目前內容用的路由（見該檔 fetchUpstreamKeywordsJson
    // 的說明）；gh.test.ts 不會打到這條路由，純粹是給 submit.test.ts 用的預設值，個別測試
    // 需要驗證最小插入的精確結果時，用 overrides 蓋掉這條拿到可控的既有內容。
    if (url.includes('raw.githubusercontent.com') && url.endsWith('/data/keywords.json')) {
      return new Response('["既有詞"]\n');
    }
